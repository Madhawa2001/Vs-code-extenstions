export type FileNode = {
  name: string;
  children?: FileNode[];
  content?: string; // for inline content files
  isDirectory?: boolean;
  fullPath?: string; // computed full path (relative)
};
...
function isTreeCharsPresent(text: string) {
  return /[├└│─]{1,}/.test(text);
}

function isDotIndentation(text: string) {
  // check if any line starts with dot(s) followed by a non-space char
  return /^\.+\S/m.test(text);
}

function detectIndentationUnit(lines: string[]) {
...
// ├── package.json
// └── README.md
function parseTreeSymbols(lines: string[]): FileNode[] {
...
//   package.json
//   README.md
function parseIndentation(lines: string[]): FileNode[] {
...
// ..package.json
// ..readme
function parseDotIndentation(lines: string[]): FileNode[] {
  const nodes: { depth: number; name: string }[] = [];
...
export function parseFileTreeText(text: string): FileNode {
  ...
  const root: FileNode = { name: ".", isDirectory: true, children: [] };
  for (const r of roots) {
    root.children!.push(r);
  }

  // Assign full relative paths to each node
  function assignPaths(node: FileNode, currentPath: string) {
